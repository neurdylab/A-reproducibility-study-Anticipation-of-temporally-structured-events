...
fpath = '/data1/neurdylab/bayrakrg/anticipation/'
header_fpath = fpath + 'pre_outputs/sub-01/sub-01_run-01_tf_func.nii.gz'
subjects = glob.glob(fpath + 'pre_outputs/*sub*')

# Create valid_vox.nii mask
find_valid_vox(fpath, subjects)
...
for sl_i in range(nSL):
    ...
    pickle.dump(sl_K,
                open(fpath + 'processed_data/perm/optimal_events_%d.p' % sl_i, 'wb'))
    pickle.dump(sl_seg,
                open(fpath + 'processed_data/perm/fit_HMM_%d.p' % sl_i, 'wb'))
    pickle.dump(sl_shift_corr,
                open(fpath + 'processed_data/perm/shift_corr_%d.p' % sl_i, 'wb'))
...
compile_optimal_events(fpath + 'processed_data/perm/', non_nan, SL_allvox,
                        header_fpath, fpath + 'processed_data/')

opt_event = nib.load(fpath + 'processed_data/optimal_events.nii').get_fdata().T
compile_fit_HMM(fpath + 'processed_data/perm/', non_nan, SL_allvox,
                header_fpath, fpath + 'processed_data/', opt_event)

compile_shift_corr(fpath + 'processed_data/perm/', non_nan, SL_allvox,
                   header_fpath, fpath + 'processed_data/')