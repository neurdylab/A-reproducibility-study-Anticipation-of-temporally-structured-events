...
def optimal_events(data_list, subjects):
    ...
    split = np.concatenate((np.full(int(len(subjects)/2), True), np.full(int(len(subjects)/2), False)))
        # creates array of half true half false, assumes even number of subjects #!!
...
def compile_fit_HMM(pickle_path, non_nan_mask, SL_allvox,
    ...
    pldiff = get_vox_map(peak_shift, SL_allvox, non_nan_mask, False)
    ...
    AUCdiff = get_vox_map(sl_AUCdiffs, SL_allvox, non_nan_mask, False)
...
def compile_shift_corr(pickle_path, non_nan_mask, SL_allvox,
    ...
    #cs, cs_q = get_vox_map(corrshift, SL_allvox, non_nan_mask) #!! indentation was wrong in initial code
    cs = get_vox_map(corrshift, SL_allvox, non_nan_mask, False)
...
def get_vox_map(SL_results, SL_voxels, non_nan_mask, return_q=True):
    ...
    if  type(SL_results[0]) == list  or SL_results[0].ndim == 1:
        nMaps = 1
    ...
    for idx, sl in enumerate(SL_voxels):
        ...
        print(sl)
        if nMaps == 1:
            print(voxel_maps[0,:,sl])
            print("test")
            print(SL_results[idx])
