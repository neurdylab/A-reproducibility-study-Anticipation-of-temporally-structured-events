...
def compile_fit_HMM(pickle_path, non_nan_mask, SL_allvox,
    ...
    pldiff  = get_vox_map(peak_shift, SL_allvox, non_nan_mask)
    ...
    AUCdiff = get_vox_map(sl_AUCdiffs, SL_allvox, non_nan_mask)
    ...
    # for p in range(nPerm):
    #     print(AUC_nonnan.shape)
    #     print(coords_nonnan.shape)
    ...
    #     K_spear[p] = spearmanr(AUC_nonnan[:,p], 90/K_nonnan)[0]
...
def compile_shift_corr(pickle_path, non_nan_mask, SL_allvox,
    ...
    cs = get_vox_map(corrshift, SL_allvox, non_nan_mask)
...
def get_vox_map(SL_results, SL_voxels, non_nan_mask, return_q=False):#True
    ...
    nVox = coords.shape[0]
    if  type(SL_results[0]) == list  or SL_results[0].ndim == 1: # SL_results[0].ndim == 1: 
         nMaps = 1
         nPerm = len(SL_results[0])
    else:
         nMaps = SL_results[0].shape[0]
         nPerm = SL_results[0].shape[1]
    #nMaps = 1 #SL_results[0].shape[0]
    #nPerm = 2    #3
