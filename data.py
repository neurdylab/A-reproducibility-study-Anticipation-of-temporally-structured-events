...
def scans_to_clips(fpath, fpath_out, subjects):
    ...
    for subj in subjects:
        ...
        for run in os.listdir(subj):
            print(run)
            if 'tf_func.nii.gz' in run:
                print('Processing ' + run)
                sub_id = run[run.find('sub'):run.find('sub')+6] # this takes subj and returns sub-01, etc
                fname = fpath_out + 'processed_data2/' + sub_id + '/' + run # preprocessed functional file
...
def find_valid_vox(fpath, subjects, min_subjs=15):#15
    ...
    for rep in range(6):
        ...
        for subj in subjects:
            ...
            #false_zero = rep_z[0][0][0] # takes top left corner, assumes that to be zero
            #nnan = ~np.all(rep_z == false_zero, axis=3)
    ...
    for subj in subjects:
        ...
        for cond in ['IN', 'SF', 'SR']:
            ...
            for i in range(6):
                ...
                #false_zero = rep_z[0][0] # takes top left corner, assumes that to be zero
